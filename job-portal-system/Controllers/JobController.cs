...
namespace job_portal_system.Controllers
{
    public class JobController : Controller
    {
        ...
        public async Task<IActionResult> Apply(ApplyJobViewModel model, IFormFile? cvFile)
        {
            ...
            // Get job and employer details for notifications
            var job = await _context.Jobs
                .Include(j => j.Employer)
                .ThenInclude(e => e.User) // IMPORTANT: Include User to get UserId
            ...
            if (job != null && job.Employer != null)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    Console.WriteLine($"Error creating employer notification: {ex.Message}");
                }
                ...
                try
                {
                    await _notificationService.CreateAdminNotificationAsync(
                        ...
                        message: $"{jobSeeker.FirstName} {jobSeeker.LastName} applied to '{job.Title}' at {job.Employer.CompanyName}",
        ...
        public async Task<IActionResult> Create(JobViewModel vm)
        {
            ...
            try
            {
                await _notificationService.CreateAdminNotificationAsync(
                    ...
                    message: $"{employer.CompanyName} posted a new job: '{job.Title}'",
            ...
            // Notify all active job seekers about new job
            try
            {
                ...
                foreach (var js in jobSeekers)
                {
                    try
                    {
                        await _notificationService.CreateNotificationAsync(
                            userId: js.UserId,
                            title: "New Job Posted",
                            message: $"Check out the new job '{job.Title}' at {employer.CompanyName}",
