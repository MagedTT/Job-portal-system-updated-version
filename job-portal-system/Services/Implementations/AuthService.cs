        ...
        public async Task<(bool IsSuccess, string ErrorMessage)> RegisterEmployerAsync(RegisterEmployerDto dto)
        {
            ...
            try
            {
                await _notificationService.CreateAdminNotificationAsync(
                    ...
                    message: $"{dto.CompanyName} has registered as an employer",
                    type: NotificationType.NewUser,
                    actionUrl: "/Admin/PendingEmployers"
                );
            }
            catch (Exception)
            {
        ...
        public async Task<(bool IsSuccess, string ErrorMessage)> RegisterJobSeekerAsync(RegisterJobSeekerDto dto)
        {
            ...
            try
            {
                await _notificationService.CreateAdminNotificationAsync(
                    ...
                    message: $"{dto.FirstName} {dto.LastName} has registered as a job seeker",
                    type: NotificationType.NewUser,
                    actionUrl: "/Admin/PendingEmployers"
                );
            }
